...
def check_resume(opt, resume_iter):
    ...
    if opt['path']['resume_state']:
        if opt['path'].get('pretrain_model_G', None) is not None or opt['path'].get('pretrain_model_E', None) is not None:
            logger.warning('pretrain_model path will be ignored when resuming training.')

        opt['path']['pretrain_model_G'] = osp.join(opt['path']['models'],
                                                   '{}_G.pth'.format(resume_iter))
        opt['path']['pretrain_model_E'] = osp.join(opt['path']['models'],
                                                   '{}_E.pth'.format(resume_iter))
        logger.info('Set [pretrain_model_G] to ' + opt['path']['pretrain_model_G'])
        logger.info('Set [pretrain_model_G] to ' + opt['path']['pretrain_model_E'])
