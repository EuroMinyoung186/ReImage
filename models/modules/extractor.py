...
class SegmentationExtractor(Extractor):
    ...
    def forward(
        ...
        image = self.pixel_decoder(latents)

        return image


def build_extractor(encoder_config, pixel_decoder_config):
    image_encoder = ImageEncoderViT(**encoder_config)
    pixel_decoder = PixelDecoder(**pixel_decoder_config)
